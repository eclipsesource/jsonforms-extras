...
class UIGridArrayRenderer implements JSONForms.IRenderer {
    ...
    private defaultGridOptions(existingOptions: uiGrid.IGridOptions,
        ...
        defaultGridOptions.onRegisterApi = (gridApi) => {
                ...
                gridApi.core.on.filterChanged(scope, () => {
                    ...
                    var terms = columns.reduce((acc, column) => {
                        ...
                        if (value !== undefined && value.length > 0) {
                            acc[column.field] = UIGridArrayRenderer.convertColumnValue(column.colDef, value);
                        }
                        return acc;
                    }, {});
            ...
            gridApi.edit.on.afterCellEdit(scope, (rowEntity, colDef:uiGrid.IColumnDef, newValue, oldValue) => {
                rowEntity[colDef.field] = UIGridArrayRenderer.convertColumnValue(colDef, newValue);
...
angular.module('jsonforms.renderers.controls.array.ui-grid').run(['RenderService', 'PathResolver', (RenderService, PathResolver) => {
    RenderService.register(new UIGridArrayRenderer(PathResolver));
}]);
